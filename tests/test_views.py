...
class TestRouteViewValidation:
    ...
    def test_missing_end_param_returns_400(self, client):
        res = client.get("/api/route/?start=Chicago, IL")
        assert res.status_code == 400

    def test_missing_start_param_returns_400(self, client):
        res = client.get("/api/route/?end=Denver, CO")
        assert res.status_code == 400

    def test_unresolvable_start_returns_400(self, client):
        with patch("route_planner.views.geocode_address", return_value=(None, None)):
            res = client.get("/api/route/?start=FakePlaceXYZ&end=Denver, CO")
        assert res.status_code == 400
        assert "start" in res.json()["error"].lower()

    def test_unresolvable_end_returns_400(self, client):
        with patch("route_planner.views.geocode_address", side_effect=[CHICAGO, (None, None)]):
            res = client.get("/api/route/?start=Chicago, IL&end=FakePlaceXYZ")
        assert res.status_code == 400
        assert "end" in res.json()["error"].lower()

    def test_routing_failure_returns_502(self, client):
    ...
    def test_no_reachable_stations_returns_422(self, client):
        with patch("route_planner.views.geocode_address", side_effect=[CHICAGO, DENVER]):
...
class TestRouteViewResponseShape:
    def _get_route(self, client, stops=None, total_cost=285.50, route=None):
        if route is None:
            route = SAMPLE_ROUTE
            ...
            with patch("route_planner.views.get_route", return_value=route):
                with patch("route_planner.views.find_stations_on_route", return_value=stop_list):
                    with patch("route_planner.views.optimize_fuel_stops",
                        ...
                        return client.get("/api/route/?start=Chicago, IL&end=Denver, CO")

    def test_returns_200(self, client):
        assert self._get_route(client).status_code == 200

    def test_response_has_all_top_level_keys(self, client):
        data = self._get_route(client).json()
        assert {"route", "map_data", "fuel_stops", "summary"} == set(data.keys())

    # Requirement: return a map of the route
    def test_map_data_is_geojson_feature_collection(self, client):
        data = self._get_route(client).json()
        assert data["map_data"]["type"] == "FeatureCollection"
        assert isinstance(data["map_data"]["features"], list)

    def test_map_data_includes_route_line(self, client):
        features = self._get_route(client).json()["map_data"]["features"]
        types = [f["properties"]["type"] for f in features]
        assert "route" in types

    def test_map_data_includes_start_and_end_markers(self, client):
        features = self._get_route(client).json()["map_data"]["features"]
        types = [f["properties"]["type"] for f in features]
        assert "start" in types
        assert "end" in types

    def test_map_data_includes_fuel_stop_markers(self, client):
        features = self._get_route(client).json()["map_data"]["features"]
        types = [f["properties"]["type"] for f in features]
        assert "fuel_stop" in types

    # Requirement: return optimal fuel stop locations
    def test_fuel_stops_contain_required_fields(self, client):
        data = self._get_route(client).json()
    ...
    def test_summary_contains_total_fuel_cost(self, client):
        data = self._get_route(client, total_cost=285.50).json()
        assert data["summary"]["total_fuel_cost"] == 285.50

    # Requirement: 10 MPG assumed
    def test_summary_fuel_efficiency_is_10_mpg(self, client):
        data = self._get_route(client).json()
        assert data["summary"]["fuel_efficiency_mpg"] == 10

    # Requirement: 500-mile max range
    def test_summary_vehicle_range_is_500_miles(self, client):
        data = self._get_route(client).json()
        assert data["summary"]["vehicle_range_miles"] == 500

    def test_summary_number_of_stops_matches_fuel_stops_list(self, client):
        data = self._get_route(client).json()
        assert data["summary"]["number_of_stops"] == len(data["fuel_stops"])

    def test_route_contains_start_and_end_location(self, client):
        data = self._get_route(client).json()
        assert "start_location" in data["route"]
        assert "end_location" in data["route"]

    def test_route_contains_distance_and_duration(self, client):
        data = self._get_route(client).json()
    ...
    def test_short_trip_returns_no_stops_but_nonzero_cost(self, client):
        short_route = {**SAMPLE_ROUTE, "distance_miles": 300.0}
        data = self._get_route(client, stops=[], total_cost=90.0, route=short_route).json()
        assert data["fuel_stops"] == []
        assert data["summary"]["total_fuel_cost"] == 90.0

    def test_total_gallons_is_sum_of_stop_gallons(self, client):
        ...
        data = self._get_route(client, stops=[stop1, stop2], total_cost=150.0).json()
